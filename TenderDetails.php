...
class TenderDetails {
	...
	public function CheckAccounts(){
		...
		$data=array("Accounts"=>$Accounts,"id"=>$_REQUEST['id'],"TenderName"=>$_REQUEST['TenderName'],"location"=>$_REQUEST['location']);
		ajaxResponse("1", $data);
...
//work here
	public function SelectChellanOfVendor(){
		...
		if($vendor_id!=""){
			$AddQry1=" AND a.vendor_id=". $vendor_id." ";
		}
		...
		$data=array("ChallanDetails"=>$result, "admin"=>$admin, "VendorList"=>$result2);
		ajaxResponse("1", $data);
	...
	public function FetchExpenses(){
		...
		$sql= "SELECT e.id,e.summary,e.report_date,e.image,e.amount,et.type AS expense_type, et.id AS expense_type_id FROM expenses e INNER JOIN expense_type et ON et.id=e.expense_type WHERE e.deleted=0 AND e.tender_id =".$_REQUEST['tender_id']  ." ORDER BY e.id DESC";
		...
		$data=array("Expenses"=>$Expenses,"ExpenseType"=>$result2, "admin"=>$admin, "Accounts"=>$Accounts);
		ajaxResponse("1", $data);
	...
	public function FilterExpenses(){
		...
		$sql= "SELECT e.id,e.summary,e.report_date,e.image,e.amount,et.type AS expense_type, et.id AS expense_type_id FROM expenses e INNER JOIN expense_type et ON et.id=e.expense_type WHERE e.deleted=0 AND e.tender_id =".$_REQUEST['tender_id']. " ".  $AddQry1 .$AddQry2 .$AddQry3. " ORDER BY e.id DESC";
		// "SELECT e.id,e.summary,e.report_date,e.image,e.amount,et.type AS expense_type, et.id AS expense_type_id        FROM expenses e INNER JOIN expense_type et ON et.id=e.expense_type WHERE e.deleted=0 AND e.tender_id =16 AND e.report_date >=2022-01-03 AND e.report_date >=2022-06-08 ORDER BY e.id DESC";
		// echo $sql;
		// die();
		$result = $this->dbc->get_result($sql);
		$ind=-1;
		...
		$data=array("Expenses"=>$Expenses, "admin"=>$admin);
		ajaxResponse("1", $data);
	...
	public function getExpenseType(){
		$sql2= "SELECT * FROM expense_type ORDER BY type";
		$result2 = $this->dbc->get_result($sql2);
		$data=array("ExpenseType"=>$result2);
		ajaxResponse("1", $data);
	}
...
// print_r($dataDelete);die();
	...
	public function viewVouchers_Id(){
		...
		$id=$_REQUEST['id'];
		$sql0= "SELECT  t.TenderName, v.expenses_date,v.expenses_number, CONCAT(u.first_name, ' ', u.last_name) AS SiteIncharge FROM expenses_voucher v INNER JOIN  tenders t ON v.tender_id=t.id INNER JOIN users u ON  t.SiteIncharge=u.id WHERE v.id='$id'";
		// echo 
		$result0 = $this->dbc->get_result($sql0);
		$TenderName=$result0[0]['TenderName'];
		...
		$data=array("Expenses"=>$result, "admin"=>$admin, "TotalAmount"=>$TotalAmount,"TenderName"=>$TenderName,"VoucherDate"=>$VoucherDate, "VoucherNumber"=>$VoucherNumber, "SiteIncharge"=>$SiteIncharge, "InWords"=>$InWords);
		ajaxResponse("1", $data);
	}
	...
	public function addBOQItem(){
		...
		if(!isset($_REQUEST['boq_amount']) || ($_REQUEST['boq_amount'] == "") ){
		...
		$data["amount"]=$_REQUEST['boq_amount'];
		$data["created_by"]=$_SESSION['USER_ID'];
	...
	public function fetchTenderID(){
		...
		$id=$_REQUEST['id'];
		$sql="SELECT t.*, CONCAT(u.first_name, ' ',u.last_name )AS SiteIncharge , CONCAT(uss.first_name, ' ',uss.last_name )AS SiteSupervisor, d.name AS Department, IFNULL(t.ContractDepositAmount,0) AS ContractDeposit FROM `tenders` t INNER JOIN users u ON t.SiteIncharge=u.id INNER JOIN users uss ON t.SiteSupervisor=uss.id INNER JOIN department d ON d.id=t.BMCDepartment WHERE t.id='$id' ";
